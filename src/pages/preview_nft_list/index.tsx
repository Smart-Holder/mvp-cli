...
class PreviewNftCard extends NavPage<{ address: string }> {
	...
	async triggerLoad() {
		...
		// 获取设备当前设置参数
		getScreenSettings(address).then(({ screenHeight, screenWidth }) => {
					...
					<Tabs tabBarBackgroundColor={"transparent"} tabBarUnderlineStyle={{ border: 0, height: '3px', background: 'linear-gradient(90deg, #4881FA, #6ED6F5)', borderRadius: '3px' }} animated={false} onChange={(e) => {
						...
						<div className="item_page" id="preview_scroll_box">
							...
							>
								...
								</div> : !loading && <Empty style={{ marginTop: '2rem', color: '#ccc' }} image={require('../../assets/empty_img.png')} description={t("暂无NFT，请添加NFT至设备")} />}
							</InfiniteScroll>
