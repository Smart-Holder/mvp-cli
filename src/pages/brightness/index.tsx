...
import { getScreenSettings, screenLight, switchAutoLight } from '../../models/device';
import './index.scss';
...
class AudioSetting extends NavPage<{ address: string }> {
	...
	render() {
		...
		return <div className="brightness_page">
			...
			<div className="setting_box">
				...
				<div className="card_box">
					<div className="top_part">
						<div className="label">{t('开关：开启/关闭自动调整屏幕亮度')}</div>
					...
					<div className="bottom_part" style={{ visibility: autoLight ? 'hidden' : 'visible', width: '100%', textAlign: 'center' }}>
						<img className="voice_low_img" src={require('../../assets/light_low.png')} />
						<div className="slider_box">
							<Slider trackStyle={{ background: 'linear-gradient(-89deg, #3076F5, #5DB7F7)', height: '.1rem' }} handleStyle={{ backgroundColor: '#fff', border: 0, boxShadow: '0px 5px 32px 0px #131212' }} max={5} min={0} value={light} dots step={1} onChange={this.sliderChange.bind(this)} />
						</div>
						<img className="voice_height_img" src={require('../../assets/light_high.png')} />
					</div>
				</div>
			</div>
		</div>
