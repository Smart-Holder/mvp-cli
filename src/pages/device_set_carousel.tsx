...
const settingDarwerConfig = [
	...
	{ label: "背景颜色", value: SettingDarwerType.color, icon: "icon-yanse" },
	{
		...
		icon: "icon-luojituxianshiyincang",
	},
...
class DeviceSetCarousel extends NavPage<Device> {
	tabsConfig = [
		{ title: this.t("轮播图片选择"), index: 0 },
		{ title: this.t("轮播时间间隔"), index: 1 },
	];
		...
		getScreenSettings(address)
			.then(
				({
					...
					this.setState({
						switchValue: switchDetails,
						volume,
						light,
						currColor: color,
						autoLight: switchAutoLight,
						time,
						versionCode,
						screenWidth,
						screenHeight,
	...
	sliderChange(type: SettingDarwerType, e: number) {
		...
		let newDsqId = setTimeout(() => {
			type === SettingDarwerType.brightness
				? screenLight(address, {
						light: light ? light : 1,
	...
	audioCard() {
		...
		return (
			<div className="setting_card_box">
				<div>{t("调整音量")}</div>
				<Slider
					max={5}
					min={0}
					...
					dots
					step={1}
					onChange={this.sliderChange.bind(this, SettingDarwerType.audio)}
				/>
			</div>
		);
	}
	...
	rotationCard() {
		...
		let { t } = this;
		return (
			<div className="setting_card_box rotation_card">
							...
							>
								...
								<div
									className={`select_btn ${
										currRotation === item.value && "select_btn_active"
									}`}
								/>
							</div>
						);
				...
				<Button
					...
					onClick={async () => {
						confirm(
							t("调整屏幕角度将重启设备，确定调整屏幕角度吗？"),
							async (isok) => {
								isok &&
									(await screenOrientation(this.params.address, currRotation));
						...
						<div
							...
							onClick={async () => {
								...
								if (item.color !== currColor)
									await screenColor(this.params.address, item.color);
							}}
						>
							<div
								className="color_bg_item"
								style={{ backgroundColor: item.color }}
							></div>
							<div className="color_text">{t(item.label)}</div>
							<div
								className={`select_btn ${
									currColor === item.color && "select_btn_active"
								}`}
							/>
						</div>
					);
	...
	async drawerItemClick(currSettingIndex: SettingDarwerType) {
		if (
			[SettingDarwerType.wifi, SettingDarwerType.version].includes(
				currSettingIndex
			)
		) {
			this.setState({
				currcallDeviceIndex: currSettingIndex,
	...
	render() {
		...
		const { t } = this;
		return (
			<div className="device_set_carousel_page">
					...
					{settingDarwerConfig.map((item) => {
						let ele = (
							<p
								...
								className={item.value == currSettingIndex ? "active" : ""}
					...
					>
						{currcallDeviceIndex === "version" ? (
							<div className="label_item">
								...
								{hasNewAction ? (
									<Button
										loading={hasNewLoading}
										className={String(hasNew && "hasNew")}
										ghost
										type="primary"
										onClick={this.settingModalClick.bind(this)}
									>
										{" "}
										{hasNew ? t("发现新版本") : t("已经是最新版本了")}
									</Button>
								) : (
									<Button
										loading={hasNewLoading}
										ghost
										type="primary"
										onClick={this.checkDeviceVersion.bind(this)}
									>
										{" "}
										{t("点击检查版本更新")}
									</Button>
							...
							<div className="label_item">
								...
								>
									{" "}
									{t(callDeviceConfig[currcallDeviceIndex]?.btnText)}
								</Button>
							</div>
						)}
					</div>
