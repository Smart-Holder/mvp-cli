...
const NftCard = (props: INftCardProps) => {
	let {
		...
		transferBtnClick,
		btnText,
		btnLoadingText = btnText,
		showChain = false,
		showTransferBtn = navigator.userAgent.includes("TokenPocket"),
	} = props;
		...
		<div className="nft_card">
			<div className="nft_info_box">
				<div className="nft_img_box">
					{Boolean(Number(nft.count) > 1) && (
						<div className="nft_count">{nft.count}</div>
					)}
					{nft.media?.match(/\.mp4/i) ? (
						<video
							controls
				...
				<div
					onClick={() =>
						props.page?.pushPage(
							`/nft_detail?token=${nft.token}&tokenId=${nft.tokenId}`
						)
					}
				>
					<div className="nft_address_box">
						<div className="nft_address_title">Address</div>
						<div className="nft_address textNoWrap">
							{getSubStr(nft.token, 19)}
						</div>
					</div>

					<div className="nft_address_box">
						<div className="nft_hash_title">Hash</div>
						<div className="nft_hash textNoWrap">
							{getSubStr(nft.tokenId, 19)}
						</div>
					</div>
					...
					{showChain && (
						<div className="nft_hash_box">
							...
							<div className="nft_hash textNoWrap">
								{
									(config.env == "prod" ? unitLabelProd : unitLabel)[
										String(nft?.chain)
									]
								}
							</div>
			...
			{!showChain && (
				<div className={`action_btn_box ${showTransferBtn && "btn_list"}`}>
					{showTransferBtn && (
						<Button
							loading={transfer_btn_disabled}
							disabled={btn_disabled || transfer_btn_disabled}
							type="primary"
							ghost={true}
					...
					<Button
						...
						disabled={transfer_btn_disabled || btn_disabled}
						onClick={btnClick}
					>
						{btn_disabled ? btnLoadingText : btnText}
					</Button>
				</div>
			)}
		</div>
